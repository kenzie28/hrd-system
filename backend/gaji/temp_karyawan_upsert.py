...
Upserts NAMA KARYAWAN / JABATAN / Lokasi Kerja / Wilayah from the gaji CSV
onto ``core.Karyawan``, creating the Karyawan (with ``level=1``) and/or the
``core.Lokasi`` if they don't exist yet.
...
def upsert_karyawan_from_gaji_row(
    *,
    ...
    if not created:
        ...
        karyawan.save(update_fields=['nama', 'jabatan', 'lokasi_kerja', 'wilayah'])
