...
# =============================================================================
# Usage:
#   ./google-cloud/deploy.sh [OPTIONS]
#
# Options:
...
#
# Examples:
#   ./google-cloud/deploy.sh                 # Simple redeploy (no rebuild, no env update)
#   ./google-cloud/deploy.sh --build         # Rebuild image and deploy
#   ./google-cloud/deploy.sh --env           # Deploy with updated env variables
#   ./google-cloud/deploy.sh --build --env   # Full deployment with rebuild and env update
#   ./google-cloud/deploy.sh --setup         # Deploy and run initial_setup on container start
#   ./google-cloud/deploy.sh --debug         # Deploy with failure-only debug output in Cloud Run logs
#   ./google-cloud/deploy.sh --build --debug # Rebuild image and deploy in debug mode
#
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Usage: ./google-cloud/deploy.sh [OPTIONS]"
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "Examples:"
            echo "  ./google-cloud/deploy.sh                 # Simple redeploy"
            echo "  ./google-cloud/deploy.sh --build         # Rebuild image and deploy"
            echo "  ./google-cloud/deploy.sh --env           # Deploy with updated env variables"
            echo "  ./google-cloud/deploy.sh --build --env   # Full deployment"
            echo "  ./google-cloud/deploy.sh --setup         # Deploy with initial data seed"
            echo "  ./google-cloud/deploy.sh --debug         # Deploy with debug logging enabled"
            exit 0
            ;;
        *)
...
if [ "$BUILD_IMAGE" = true ]; then
    ...
    docker build -f "${SCRIPT_DIR}/Dockerfile" -t "${TAGGED_IMAGE}" -t "${LATEST_IMAGE}" "${REPO_ROOT}"
